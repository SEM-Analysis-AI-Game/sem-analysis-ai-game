 ...
 * array.
 */
export function drawImage(state: State): void {
...
function setSegment<StateType extends State>(
  ...
  if (
    ...
    if (updateDrawing) {
      fillImagePixel(state, pos, segment, numNeighbors < 4);
    }
...
function setNonBoundarySegment<StateType extends State>(
  ...
  setSegment(onUpdateSegment, state, pos, 4, segment, updateDrawing);
}
...
function setBoundarySegment<StateType extends State>(
          ...
          if (updatedNeighbor) {
            if (updateDrawing) {
              fillImagePixel(
                state,
                neighborPos,
                neighborData.segment,
                neighborData.inSegmentNeighbors < 4
              );
            }
  ...
  setSegment(onUpdateSegment, state, pos, numNeighbors, segment, updateDrawing);
}
 ...
 * @param updateDrawing Whether or not to update the images pixels.
 */
 ...
 *
 * @param onUpdateSegment Called when a pixel is flood filled.
 */
...
>(
  ...
  for (const fill of fills) {
    ...
    if (data && data.segment !== fill.segment) {
      ...
      throw Error("Flood fill starting point is not in the correct segment");
    }
  }
}
